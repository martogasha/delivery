...
@include('flash-message')
        ...
        <input type="hidden" id="Carrier_id" value="{{$booking->carrier_id}}">
        <input type="hidden" id="bookings" value="{{$booking->id}}">
                ...
                <div class="subsite">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="mr-car-box-wrap" id="carrierDesc">
                                <div class="mr-car-box" id="searchBanner">
                                    <h5>Searching for Nearby Carriers...</h5>
                                </div>
                                </div>
                        </div>
                    </div>
...
<script>
    ...
    channel.bind('my_search', function(data) {
        $.ajax({
            type:"get",
            url:"{{url('carrierSearch')}}",
            data:{'carrier_id':data.carrier_id,'booking_id':$('#bookings').val()},
            success:function (data) {
                $('#carrierDesc').html(data);
    ...
    $(document).ready(function () {
        ...
        if (carrier_id){
            $.ajax({
                type:"get",
                url:"{{url('displayCarrier')}}",
                data:{'carrier_id':carrier_id,'booking_id':$('#bookings').val()},
                success:function (data) {
                    $('#searchBanner').hide();
                    $('#carrierDesc').html(data);
